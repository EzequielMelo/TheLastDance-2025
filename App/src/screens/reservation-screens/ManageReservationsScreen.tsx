...
export default function ManageReservationsScreen({
  ...
  const handleApproveReservation = async (reservationId: string) => {
    ...
    if (processingReservationId === reservationId) {
      console.log("⚠️ Ya se está procesando esta reserva, ignorando...");
      return;
    }
    ...
    setAlertOnClose(() => async () => {
      try {
        ...
        await ReservationsService.updateReservationStatus(
          reservationId,
          "rejected",
          "Reserva rechazada por el restaurante",
        );
        // Mostrar mensaje de éxito
        setAlertTitle("Reserva Rechazada");
        setAlertMessage(
          "La reserva ha sido rechazada y el cliente será notificado",
        );
        setAlertType("success");
        setAlertOnClose(() => () => {
          setShowAlert(false);
            ...
            {filteredReservations.length === 0 ? (
              <View style={{ paddingVertical: 48, alignItems: "center" }}>
                ...
                >
                  No hay reservas
                  {filter !== "all"
                    ? ` ${getStatusText(filter).toLowerCase()}s`
                    : ""}
                </Text>
              </View>
            ) : (
                  ...
                  <View style={{ gap: 10 }}>
                    ...
                    {reservation.table && (
                      ...
                      >
                        ...
                        >
                          Mesa {reservation.table.number} (Cap:{" "}
