...
export class ReservationsService {
  ...
  static async getClientUpcomingReservation(userId: string): Promise<{
    ...
    for (const reservation of reservations) {
      ...
      if (reservationDateTime <= in45Minutes) {
        const minutesUntil = Math.floor(
          (reservationDateTime.getTime() - nowArgentina.getTime()) /
            (60 * 1000),
        );
    ...
    console.log(
      "❌ No hay reservas dentro de los próximos 45 minutos ni pasadas",
    );
    return null;
  }
  ...
  static async createReservation(
    ...
    if (table.capacity < data.party_size) {
      throw new Error(
        `La mesa seleccionada solo tiene capacidad para ${table.capacity} personas`,
      );
    }
    ...
    if (reservationDateTime < minReservationTime) {
      throw new Error(
        "La reserva debe ser con al menos 15 minutos de anticipación",
      );
    }
    ...
    if (user && tableData) {
      try {
        ...
        console.error(
          "❌ Error enviando notificación de nueva reserva:",
          notificationError,
        );
  ...
  static async updateTableOccupiedStatus(
    ...
    console.log(
      `Reserva ${isOccupied ? "aprobada" : "liberada"} para mesa ${tableId} el ${reservationDate} a las ${reservationTime}`,
    );
  ...
  static async updateReservationStatus(
    ...
    if (data.status === "approved") {
      try {
        ...
        console.log(
          `✅ Mesa ${reservationDetails.table_id} marcada como ocupada para ${reservationDetails.date} ${reservationDetails.time}`,
        );
      } catch (occupiedError) {
        console.error(
          "Error actualizando estado de ocupación de la mesa:",
          occupiedError,
        );
    ...
    try {
      const { sendReservationApprovedEmail, sendReservationRejectedEmail } =
        await import("../../lib/emails");

      const userName =
        `${reservationDetails.user?.first_name || ""} ${reservationDetails.user?.last_name || ""}`.trim();
      const userEmail = reservationDetails.user?.email;

      if (!userEmail) {
        console.warn(
          "No se pudo enviar email: email del usuario no encontrado",
        );
        return reservation;
      }
      ...
      if (data.status === "approved") {
        const tableNumber =
          reservationDetails.table?.number?.toString() || "N/A";
        ...
        await sendReservationRejectedEmail(
          ...
          data.rejection_reason ||
            "La reserva no pudo ser procesada en este momento.",
        );
      }
    } catch (emailError) {
  ...
  static async checkTableAvailability(
    ...
    console.log(
      `Found ${existingReservations?.length || 0} existing reservations for table ${tableId} on ${date}`,
    );
    ...
    console.log(
      `Generated ${slots.length} slots, ${slots.filter(s => s.available).length} available`,
    );
  ...
  static async checkAvailability(
    ...
    console.log(
      `Found ${tables?.length || 0} tables suitable for ${data.party_size || "any"} people`,
    );
    ...
    console.log(
      `Found ${existingReservations?.length || 0} existing reservations for ${data.date}`,
    );
  ...
  static async isTableReserved(
    ...
    try {
      ...
      console.log(
        `Verificando reservas para mesa ${tableId} el ${date} entre ${startTime} y ${endTime}`,
      );
      ...
      if (hasReservation) {
        console.log(
          `Mesa ${tableId} tiene ${reservations.length} reserva(s) en la ventana de tiempo`,
        );
      }
  ...
  static async getReservationById(
    ...
    if (user) {
      try {
        const { data: authUser, error: authError } =
          await supabase.auth.admin.getUserById(reservation.user_id);
      ...
      console.error(
        "❌ No se pudo obtener usuario de la tabla users, no se puede obtener email",
      );
    }
    ...
    if (reservation.approved_by) {
      ...
      if (!approverError && approver) {
        ...
        try {
          const { data: authApprover } = await supabase.auth.admin.getUserById(
  ...
  static async getAvailableTablesForReservation(
    ...
    console.log("Getting available tables for:", {
      date,
      time,
      tableType,
      partySize,
    ...
    console.log(
      `Found ${tables.length} tables of type ${tableType} with capacity >= ${partySize}`,
    );
    ...
    for (const table of tables) {
      const isAvailable = await this.isTableAvailableAtTime(
        table.id,
        date,
        time,
      );
    ...
    console.log(
      `${availableTables.length} tables are available at ${time} on ${date}`,
    );
  ...
  static async suggestAlternativeTimes(
    ...
    console.log("🕐 Buscando horarios alternativos para:", {
      date,
      time,
      tableType,
      partySize,
    ...
    console.log(
      `✅ Encontrados ${suggestions.length} horarios alternativos:`,
      suggestions,
    );
  ...
  private static async isTableAvailableAtTime(
    ...
    for (const reservation of reservations) {
      ...
      if (
        requestedTimeInMinutes >= blockStart &&
        requestedTimeInMinutes <= blockEnd
      ) {
  ...
  static async checkAndActivateReservation(userId: string): Promise<{
    ...
    try {
      ...
      console.log(
        `🔍 [checkAndActivateReservation] Usuario: ${userId}, Fecha: ${dateString}, Hora: ${currentHour}:${String(currentMinute).padStart(2, "0")}`,
      );
      ...
      if (quickCheck) {
        console.log(
          `⚡ Usuario ya tiene waiting_list en estado ${quickCheck.status} - skip`,
        );
      ...
      for (const reservation of reservations) {
        ...
        const reservationTimeInMinutes =
          (resHours ?? 0) * 60 + (resMinutes ?? 0);

        // Ventana de activación: -45min de la hora de reserva
        const activationTime = reservationTimeInMinutes - 45;

        console.log(
          `📅 Reserva ${reservation.id} - Hora: ${reservation.time}, Activación: ${Math.floor(activationTime / 60)}:${(activationTime % 60).toString().padStart(2, "0")}`,
        );
        ...
        if (currentTimeInMinutes >= activationTime) {
          ...
          const { data: existingWaiting, error: waitingCheckError } =
            await supabase
              .from("waiting_list")
              .select("id, status")
              .eq("client_id", userId);
          ...
          if (waitingCheckError) {
            console.error(
              "❌ Error verificando waiting_list:",
              waitingCheckError,
            );
            continue;
          }
          ...
          const { data: table, error: tableError } = await supabase
            .from("tables")
            .select(
              "id, number, capacity, type, is_occupied, id_client, table_status",
            )
            .eq("id", reservation.table_id)
            .single();
          ...
          if (
            table.is_occupied ||
            (table.id_client && table.id_client !== userId)
          ) {
            console.log(
              `⚠️  Mesa no disponible (occupied: ${table.is_occupied}, client: ${table.id_client})`,
            );
            continue;
          }
          ...
          const { data: waitingEntry, error: waitingInsertError } =
            await supabase
              .from("waiting_list")
              .insert({
                client_id: userId,
          ...
          console.log(
            `✅ Reserva activada - waiting_list creado (NO se asigna id_client para no pisar cliente actual)`,
          );
          ...
          console.log(
            `⏰ Falta ${Math.floor(minutesUntil / 60)}h ${minutesUntil % 60}m para activar`,
          );
        }
      }
        ...
        if (currentTimeInMinutes > expirationTime) {
          ...
          if (waitingEntries && waitingEntries.length > 0) {
            ...
            if (deleteWaitingError) {
              console.error(
                "❌ Error eliminando waiting_list:",
                deleteWaitingError,
              );
            } else {
              console.log(
                `✅ waiting_list ${waitingEntry?.id} eliminado (llegó temprano pero nunca confirmó)`,
              );
            }
            ...
            if (freeTableError) {
              ...
              console.log(
                `✅ Mesa ${reservation.table_id} liberada (llegó temprano pero nunca confirmó)`,
              );
            }
            ...
            if (cancelReservationError) {
              ...
              console.log(
                `✅ Reserva ${reservation.id} cancelada por no confirmación`,
              );
            }
          } else {
            ...
            if (
              !anyWaitingList ||
              !["seated", "completed"].includes(anyWaitingList.status)
            ) {
              console.log(
                `⏰ Usuario nunca confirmó su llegada - cancelando reserva ${reservation.id}`,
              );
              ...
              if (cancelReservationError) {
                ...
                console.log(
                  `✅ Reserva ${reservation.id} cancelada por ausencia (nunca llegó)`,
                );
              }
            } else {
              console.log(
                `ℹ️  Reserva ${reservation.id} ya fue completada (status: ${anyWaitingList.status})`,
              );
            }
          }
        }
